...
TEXT_SHEBANG_PREFIX_RE = re.compile(
    r"^#!" + re.escape(TERMUX_PREFIX) + r"/bin/([^\n\r/]+)"
)
    ...
    for chunk in raw.split(","):
        ...
        selected = next(
            (
                name
                for name in options
                if name not in IGNORED_DEPENDENCIES
            ),
            options[0],
        )
...
def resolve_dependency_closure(
    ...
    while pending:
        ...
        if record is None:
            raise KeyError(
                f"Termux package '{name}' was not found in the package index"
            )
        ...
        for dependency in record.depends:
            if (
                ...
                and dependency not in IGNORED_DEPENDENCIES
            ):
...
def load_data_tar_bytes_from_deb(
    payload: bytes,
) -> tuple[bytes, str]:
    ...
    raise FileNotFoundError(
        "data.tar member not found in deb archive"
    )
...
def normalize_text_shebang(
    ...
    suffix = first[
        first.find(match.group(0))
        + len(match.group(0)):
    ]

    lines[0] = (
        f"#!/usr/bin/env {interpreter}{suffix}"
    )

    return "".join(lines).encode("utf-8")


def strip_termux_prefix(
    path: str,
) -> str | None:
    normalized = path.lstrip("./")

    prefix = (
        TERMUX_PREFIX.lstrip("/")
        + "/"
    )
...
def _normalize_asset_link_path(
    path: str,
) -> str:
    return (
        str(path)
...
def serializable_manifest(
    ...
    normalized_links = [
        {
            **link,
            "path": _normalize_asset_link_path(
                link.get("path", "")
            ),
            "target": _normalize_asset_link_path(
                link.get("target", "")
            ),
        }
        for link in links
    ...
    normalized_files = sorted(
        dict.fromkeys(
            normalized
            for normalized in (
                _normalize_asset_link_path(file_path)
