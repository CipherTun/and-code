...
class AntigravityTranscriptAdapter(
    ...
    fun parseTranscript(sessionId: String, file: File): AntigravityTranscriptResult {
        ...
        file.forEachLine { line ->
            ...
            messages += OpenCodeMessage(
                info = OpenCodeMessageInfo(messageId, sessionId, role, OpenCodeTime(System.currentTimeMillis(), System.currentTimeMillis()), agent = "antigravity"),
                parts = listOf(OpenCodePart(id = "$messageId-part", sessionId = sessionId, messageId = messageId, type = "text", text = text)),
            )
        }
        return AntigravityTranscriptResult(messages, events)
    }

    fun parseHooks(sessionId: String, lines: Sequence<String>): List<OpenCodeEvent> = buildList {
        lines.forEach { line ->
            ...
            when (record.event) {
                "PermissionAsked", "PermissionAsked:PreToolUse" -> {
                    ...
                    add(OpenCodeEvent.PermissionAsked(PermissionRequest(id, sessionId, record.toolName ?: "tool", metadata = record.toolArgs?.let { mapOf("arguments" to it) } ?: emptyMap())))
                }
    ...
    companion object {
        fun hookJson(record: AntigravityHookRecord): String = Json { encodeDefaults = true }.encodeToString(AntigravityHookRecord.serializer(), record)
    }
}
