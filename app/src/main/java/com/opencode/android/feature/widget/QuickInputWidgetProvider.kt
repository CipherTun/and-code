...
class QuickInputWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        const val ACTION_SEND = "com.opencode.android.widget.SEND"
        const val ACTION_MIC = "com.opencode.android.widget.MIC"
        const val EXTRA_TEXT = "widget_text"

        fun updateWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            val views = RemoteViews(context.packageName, R.layout.widget_quick_input)

            val sendIntent = Intent(context, QuickInputActivity::class.java).apply {
                action = ACTION_SEND
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
            }
            val sendPendingIntent = PendingIntent.getActivity(
                context, appWidgetId, sendIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_MUTABLE
            )

            val micIntent = Intent(context, QuickInputActivity::class.java).apply {
                action = ACTION_MIC
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
            }
            val micPendingIntent = PendingIntent.getActivity(
                context, appWidgetId + 1000, micIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_MUTABLE
            )
