...
object AssistantProfileResolver {
    ...
    ): Result<ResolvedAssistantProfile> =
        runCatching {
            val connection =
                connections.firstOrNull { it.id == profile.backendId }
    ...
    fun resolveSessionId(profile: AssistantProfile): String? = profile.sessionId.takeIf { profile.continuousConversation }
}
