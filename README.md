...
- サブエージェント（taskツール）のセッションをチャットから開き、戻るボタンまたは端末の戻る操作でメインエージェントに復帰
...
- **チャット**: 会話、reasoning・ツール実行・コマンド出力の折りたたみ表示、音声入力、モデル切り替え、承認操作、サブエージェントのセッションの表示と復帰、別の実行先へのハンドオフ
...
**作業先**画面の**LANで検索**を押すと、mDNS（NSD）で約10秒間LAN上の`_opencode._tcp.`サービスを検索し、見つかったサーバーを一覧表示します。タップすると接続先の入力欄へURLと名前が自動入力されます。

`opencode serve`自体はmDNSで自己広告しないため、PC側で別途アドバタイズを行ってください。

Linux（avahi）:

```bash
avahi-publish -s "My PC OpenCode" _opencode._tcp 4096
```

macOS:

```bash
dns-sd -R "My Mac OpenCode" _opencode._tcp . 4096
```
